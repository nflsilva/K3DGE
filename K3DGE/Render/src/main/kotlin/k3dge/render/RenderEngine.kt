...
class RenderEngine(configuration: EngineConfiguration) {
    ...
    fun render2D(texture: Texture, transform: TransformData){
        renderer2D.renderSprite(Sprite(SpriteSizeEnum.X1, texture), transform)
    }
    fun render2D(sprite: Sprite, transform: TransformData){
        renderer2D.renderSprite(sprite, transform)
    }
    fun render2D(particle: Point, transform: TransformData){
        renderer2D.renderPoint(particle, transform)
    }
